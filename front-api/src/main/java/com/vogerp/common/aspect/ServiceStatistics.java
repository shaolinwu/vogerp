...
import com.vogerp.monitor.PerfMonitor;
...
public class ServiceStatistics {
	...
	// 定义一个日志的全局的静态
	private static final Logger logger = LoggerFactory.getLogger(ServiceStatistics.class);
	 ...
	 */
	static final String pCutStr = "execution(* com.vogerp.*.service.*.*(..))";
...
//			logger.debug("耗时: " + total + " ms!");
			...
			if (!PerfMonitor.getDefaultKPICollector().hasKPI(methodName)) {
				PerfMonitor.getDefaultKPICollector().addAPI(methodName);
			}
			PerfMonitor.getDefaultKPICollector().updateAPI(methodName, total);
			...
			PerfMonitor.getDefaultKPICollector().updateAPIWithFail(methodName, total);
	...
	@Before(value = "execution(* com.vogerp.*.service.*.*(..))")
	...
	@AfterReturning(value = "execution(* com.vogerp.*.service.*.*(..))")
