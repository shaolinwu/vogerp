...
public class OrderPaymentWithMasterUI extends HTMLWidgetType {
    ...
    public OrderPaymentWithMasterUI(String id)
    {
        super(id);
    }
	...
	public static long getOrgId(final IRentOrLoanOrder order) {
	...
	public void generateEndHTML(UserRequestContext context, UIFormObject ownerEntity, int depth) {
		...
		try {
			...
			context.generateHTML("<tr><th>\u9700\u6C42\u65B9: "+CustomerInfoUtil.getCustomerEnterpriseBasicInfo(provider)+"</th><th>\u63A5\u5355\u5E08\u5085: "+CustomerInfoUtil.getCustomerEnterpriseBasicInfo(takener)+"</th></tr>");
			context.generateHTML("<tr><td>\u5730\u5740: "+deliveryInfo.getAddress()+"</td><td></td></tr>");
			...
			if (state == PayOrderStatusType.PAYED) {
				context.generateHTML("<tr class=\"order_step\">");
				if (isCurrProvider) {
					context.generateHTML("<td>2.<button onclick=\""+funcRef+"ensurePayMasterOrder(defaultname." + htmlId + ");\" class=\"ui-btn-inline\">\u786E\u8BA4\u5B8C\u6210</button></td><td>2.\u89E3\u51B3\u95EE\u9898\u4E2D</td>");
				} else {
					context.generateHTML("<td>2.\u7B49\u5F85\u786E\u8BA4\u5B8C\u6210</td>2.\u89E3\u51B3\u95EE\u9898\u4E2D<td></td>");
				}
				context.generateHTML("</tr>");
			} else {
				context.generateHTML("<tr class=\"\">");
				context.generateHTML("<td>2." + order.getTakenStatus().getDisplayName() + "</td><td>2.\u95EE\u9898\u672A\u89E3\u51B3</td>");
				context.generateHTML("</tr>");
			}
			...
			if (order.getTakenStatus() == OrderStatusType.TAKEN_COMPLETED) {
				context.generateHTML("<tr class=\"order_step\">");
				if (isCurrTaker) {
					context.generateHTML("<td>3.\u8BC4\u4EF7\u4FE1\u606F</td><td>5.<button onclick=\""+funcRef+"commentOrder(defaultname." + htmlId + ");\" class=\"ui-btn-inline\">\u8BC4\u4EF7</button></td>");
				} else {
					context.generateHTML("<td>3.<button onclick=\""+funcRef+"commentOrder(defaultname." + htmlId + ");\" class=\"ui-btn-inline\">\u8BC4\u4EF7</button></td><td>5.\u8BC4\u4EF7\u4FE1\u606F</td>");
				}
				context.generateHTML("</tr>");
			} else {
				context.generateHTML("<tr class=\"\">");
				context.generateHTML("<td>3.\u8BC4\u4EF7\u4FE1\u606F</td><td>3.\u8BC4\u4EF7\u4FE1\u606F</td>");
				context.generateHTML("</tr>");
			}
